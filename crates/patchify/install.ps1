...
$Bin = "patchify"
$InstallDir = if ($env:PATCHIFY_INSTALL_DIR) { $env:PATCHIFY_INSTALL_DIR } else { "$env:USERPROFILE\.local\bin" }

function Fail($message) {
    Write-Error "patchify-installer: ERROR: $message"
    exit 1
}
...
# Version resolution.
if ($env:PATCHIFY_VERSION) {
    $Version = $env:PATCHIFY_VERSION
} else {
    Write-Host "patchify-installer: resolving latest release..."
    try {
        $Release = Invoke-RestMethod -Uri "https://api.github.com/repos/$Repo/releases/latest" -ErrorAction Stop
        $Version = $Release.tag_name
    } catch {
        Fail "could not determine latest release (set `$env:PATCHIFY_VERSION to pin one): $_"
    }
}
...
try {
    ...
    Write-Host "patchify-installer: verifying checksum..."
    ...
    Write-Host "patchify-installer: extracting..."
    ...
    Write-Host "patchify-installer: installed $Version to $InstallDir\$Bin.exe"
    if ($env:PATH -notlike "*$InstallDir*") {
        Write-Host "patchify-installer: NOTE: $InstallDir is not on your PATH"
    }
} finally {
