...
$Bin = "shellaborate"
$InstallDir = if ($env:SHELLABORATE_INSTALL_DIR) { $env:SHELLABORATE_INSTALL_DIR } else { "$env:USERPROFILE\.local\bin" }

function Fail($message) {
    Write-Error "shellaborate-installer: ERROR: $message"
    exit 1
}
...
# Version resolution.
if ($env:SHELLABORATE_VERSION) {
    $Version = $env:SHELLABORATE_VERSION
} else {
    Write-Host "shellaborate-installer: resolving latest release..."
    try {
        $Release = Invoke-RestMethod -Uri "https://api.github.com/repos/$Repo/releases/latest" -ErrorAction Stop
        $Version = $Release.tag_name
    } catch {
        Fail "could not determine latest release (set `$env:SHELLABORATE_VERSION to pin one): $_"
    }
}
...
try {
    Write-Host "shellaborate-installer: downloading $Version for $Target..."
    ...
    Write-Host "shellaborate-installer: verifying checksum..."
    ...
    Write-Host "shellaborate-installer: extracting..."
    ...
    Write-Host "shellaborate-installer: installed $Version to $InstallDir\$Bin.exe"
    if ($env:PATH -notlike "*$InstallDir*") {
        Write-Host "shellaborate-installer: NOTE: $InstallDir is not on your PATH"
    }
} finally {
