#!/bin/sh
# readcursively installer — POSIX, fail-closed.
# Usage: curl -fsSL https://raw.githubusercontent.com/riceharvest/agents-first-toolkit/main/install.sh | sh
# Override version: READCURSIVELY_VERSION=v0.1.0 sh install.sh
set -eu

REPO="riceharvest/agents-first-toolkit"
BIN="readcursively"
INSTALL_DIR="${READCURSIVELY_INSTALL_DIR:-$HOME/.local/bin}"

log() { printf 'readcursively-installer: %s\n' "$1" >&2; }
...
if [ -n "${READCURSIVELY_VERSION:-}" ]; then
  VERSION="$READCURSIVELY_VERSION"
else
  log "resolving latest release..."
  VERSION=$(fetch "https://api.github.com/repos/$REPO/releases/latest" |
    sed -n 's/.*"tag_name": *"\([^"]*\)".*/\1/p' | head -n 1)
  [ -n "$VERSION" ] || fail "could not determine latest release (set READCURSIVELY_VERSION to pin one)"
fi
