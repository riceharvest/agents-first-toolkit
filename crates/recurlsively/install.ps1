...
# Override version: $env:RECURSIVELY_VERSION = "v0.1.0"
$ErrorActionPreference = "Stop"

$Repo = "riceharvest/agents-first-toolkit"
$Bin = "recurlsively"
$InstallDir = if ($env:RECURSIVELY_INSTALL_DIR) { $env:RECURSIVELY_INSTALL_DIR } else { "$env:USERPROFILE\.local\bin" }

function Fail($message) {
    Write-Error "recurlsively-installer: ERROR: $message"
    exit 1
}
...
# Version resolution.
if ($env:RECURSIVELY_VERSION) {
    $Version = $env:RECURSIVELY_VERSION
} else {
    Write-Host "recurlsively-installer: resolving latest release..."
    try {
        $Release = Invoke-RestMethod -Uri "https://api.github.com/repos/$Repo/releases/latest" -ErrorAction Stop
        $Version = $Release.tag_name
    } catch {
        Fail "could not determine latest release (set `$env:RECURSIVELY_VERSION to pin one): $_"
    }
}
...
try {
    Write-Host "recurlsively-installer: downloading $Version for $Target..."
    ...
    Write-Host "recurlsively-installer: verifying checksum..."
    ...
    Write-Host "recurlsively-installer: extracting..."
    ...
    Write-Host "recurlsively-installer: installed $Version to $InstallDir\$Bin.exe"
    if ($env:PATH -notlike "*$InstallDir*") {
        Write-Host "recurlsively-installer: NOTE: $InstallDir is not on your PATH"
    }
} finally {
