...
# Override version: RECURSIVELY_VERSION=v0.1.0 sh install.sh
set -eu

REPO="riceharvest/agents-first-toolkit"
BIN="recurlsively"
INSTALL_DIR="${RECURSIVELY_INSTALL_DIR:-$HOME/.local/bin}"

log() { printf 'recurlsively-installer: %s\n' "$1" >&2; }
...
if [ -n "${RECURSIVELY_VERSION:-}" ]; then
  VERSION="$RECURSIVELY_VERSION"
else
  log "resolving latest release..."
  VERSION=$(fetch "https://api.github.com/repos/$REPO/releases/latest" |
    sed -n 's/.*"tag_name": *"\([^"]*\)".*/\1/p' | head -n 1)
  [ -n "$VERSION" ] || fail "could not determine latest release (set RECURSIVELY_VERSION to pin one)"
fi
